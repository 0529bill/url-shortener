const handleValidation = ({ errorArray = [], dataSource = [], defaultErrorMessage = "", }) => {
    ...
    errorArray.forEach((item) => {
        if (!Array.isArray(item?.condition) && !item?.customCondition) {
            return console.error(`Invalid type \`${typeof !item?.condition}"\` supplied to parameter "condition", expected \`array\`;
        ...
        if (typeof item?.index !== "string") {
            ...
            return console.error(`Invalid type \`${typeof item?.index}"\` supplied to parameter "index", expected \`string\`!`);
        }
        // 有傳customValidate的情況
        if (item?.customCondition) {
            ...
            if (!Array.isArray(conditionReturnArray)) {
                // eslint-disable-next-line no-console
                return console.error(`Invalid type \`${typeof conditionReturnArray}"\` return from parameter "customValidation", expected \`array\`!`);
            }
            if (conditionReturnArray.length) {
                const multiple = conditionReturnArray.length > 1;
                let returnObject = multiple ? [] : "";
                ...
                conditionReturnArray.forEach((result, index) => {
                    ...
                    if (item?.errorFormat && errorResult) {
                        if (!Array.isArray(item?.errorFormat)) {
                            // eslint-disable-next-line no-console
                            return console.error(`Invalid type \`${typeof item?.errorFormat}"\` return from parameter "errorFormat", expected \`array\`!`);
                        }
